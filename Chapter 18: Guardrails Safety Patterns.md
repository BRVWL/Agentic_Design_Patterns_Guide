...
try: # If the output is a TaskOutput object, extract its pydantic model content
...
try: # Kickoff the crew with the user input.
...
Furthermore, utilizing a less computationally intensive model like Gemini Flash Lite as an additional safeguard, employing isolated code execution environments, conducting thorough evaluation and monitoring of agent actions, and restricting agent activity within secure network boundaries like VPC Service Controls are essential practices. Prior to implementing these safety measures, a detailed risk assessment specific to the agent's functionalities, domain, and deployment environment is necessary. In addition to technical safeguards, it is important to sanitize all model-generated content before display in user interfaces to prevent potential execution of malicious code in browsers.
...
Callbacks, for instance, can utilize an LLM like Gemini to power these robust, prompt-based safety measures.This strategy mitigates risks related to content safety, agent misalignment, and brand safety arising from potentially unsafe user and tool inputs. A fast, cost-effective LLM, like Gemini Flash, can screen these inputs. An example prompt directs an LLM to function as a safety guardrail. This example contains a **Jailbreak** attempt, which involves crafting specialized prompts to make a Large Language Model bypass its safety features and ethical restrictions. The goal is to trick the AI into generating content it is programmed to refuse, such as harmful instructions, malicious code, or offensive material. Essentially, it is a form of adversarial attack that exploits loopholes in the AI's programming to make it violate its own rules.
...
### Engineering Reliable Agents: Applying Proven Software Principles
...
![Guardrail design pattern](./imgs/unnamed-32.png)
_Guardrail design pattern_

## Key Takeaways

- Guardrails are essential for building responsible, ethical, and safe AI systems by preventing harmful, biased, or off-topic responses.
...
- Effective guardrails are crucial for maintaining user trust and protecting the reputation of the AI system and its developers.
- The most effective way to build reliable, production-grade AI agents is to treat them as complex software, applying the same proven engineering best practices—like fault tolerance, state management, and robust testing—that have governed traditional systems for decades.

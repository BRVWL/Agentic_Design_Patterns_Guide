...
The Model Context Protocol operates on a clear client-server model that standardizes the flow of information and actions. Understanding the interaction between the core components is key to grasping how MCP enables advanced agentic behavior.
...
2.  **Request Formulation:** The LLM determines it needs to use one of the discovered tools. For instance, it decides to send an email. It formulates a request, specifying the tool to use (`send_email`) and the necessary parameters (recipient, subject, body).
...
The Model Context Protocol significantly broadens the capabilities of AI agents and LLMs, making them more versatile and powerful in real-world applications. Here are eight key use cases for MCP:

1.  **Database Integration:** MCP allows LLMs and agents to seamlessly access and interact with structured data in databases. For instance, using the MCP Toolbox for Databases, an agent can query BigQuery datasets to retrieve real-time information, generate reports, or update records, all driven by natural language commands.
2.  **Generative Media Orchestration:** MCP enables agents to integrate with advanced generative media services. Through MCP Tools for Genmedia Services, an agent can orchestrate workflows involving Google's Imagen for image generation, Google's Veo for video creation, Google's Chirp 3 HD for realistic voices, or Google's Lyria for music composition, allowing for dynamic content creation within AI applications.
3.  **External API Interaction:** MCP provides a standardized way for LLMs to call and receive responses from any external API. This means an agent can fetch live weather data, pull stock prices, send emails, or interact with CRM systems, extending its capabilities far beyond its core language model.
4.  **Reasoning-Based Information Extraction:** Leveraging an LLM's strong reasoning skills, MCP facilitates effective, query-dependent information extraction that surpasses conventional search and retrieval systems. Instead of a traditional search tool returning an entire document, an agent can analyze the text and extract the precise clause, figure, or statement that directly answers a user's complex question.
5.  **Custom Tool Development:** Developers can build custom tools and expose them via an MCP server (e.g., using FastMCP). This allows specialized internal functions or proprietary systems to be made available to LLMs and other agents in a standardized, easily consumable format, without needing to modify the LLM directly.
6.  **Standardized LLM-to-Application Communication:** MCP ensures a consistent communication layer between LLMs and the applications they interact with. This reduces integration overhead, promotes interoperability between different LLM providers and host applications, and simplifies the development of complex agentic systems.
...
8.  **IoT Device Control:** MCP can facilitate LLM interaction with Internet of Things (IoT) devices. An agent could use MCP to send commands to smart home appliances, industrial sensors, or robotics, enabling natural language control and automation of physical systems.
9.  **Financial Services Automation:** In financial services, MCP could enable LLMs to interact with various financial data sources, trading platforms, or compliance systems. An agent might analyze market data, execute trades, generate personalized financial advice, or automate regulatory reporting, all while maintaining secure and standardized communication.

The Model Context Protocol (MCP) facilitates agent access to real-time information by enabling connections to databases, APIs, and web resources. This protocol further allows agents to perform actions, such as sending emails, updating records, or controlling physical devices. Agents can also execute complex tasks by integrating data from various sources, processing this information, and then initiating actions across different platforms. Additionally, MCP supports integration with media generation tools, facilitating the use of image, video, and audio generation within AI applications.

## Hands-On Code Example

This section outlines how to connect to a local Model Context Protocol (MCP) server that provides file system operations, enabling a Google Agent Development Kit (ADK) agent to interact with the local file system.
...
Finally: Run `adk web` and Get Started!
...
Once the ADK Web UI loads in your browser, here's what to do: Pick the `filesystem_assistant_agent` from the agent menu. Then, try it with prompts like "Show me what's in this folder," "Can you read the sample.txt file?" (assuming you've got one in `TARGET_FOLDER_PATH`), or "What's inside another_file.md?"

### Creating an MCP Server with FastMCP

This example illustrates the rapid setup of a Model Context Protocol (MCP) server using the FastMCP library. The server provides a basic "greet" tool. Once active, ADK agents or other MCP clients can communicate with this tool via HTTP.
...
Creating an `__init__.py` file within the `fastmcp_client_agent` directory is necessary. This ensures the agent is recognized as a discoverable Python package for the Agent Development Kit (ADK).

Start the FastMCP server by executing `python fastmcp_server.py` in a separate terminal. Then, navigate to the parent directory of `fastmcp_client_agent` (e.g., `adk_agent_samples`) in your terminal and run `adk web`. Once the ADK Web UI appears in your browser, select the `fastmcp_greeter_agent` from the agent menu. You can then test it with a prompt such as "Greet John Doe." The agent will use the `greet` tool on your FastMCP server to generate a response.
...
![Model Context protocol](./imgs/unnamed-12.png)
...
- The Model Context Protocol (MCP) is an open standard facilitating standardized communication between Large Language Models (LLMs) and external applications, data sources, and tools.
...
- MCP Toolbox for Databases enables agents to access database data, with integrated ADK support.
...
- [MCP Tools for Genmedia Services](https://google.github.io/adk-docs/mcp/#mcp-servers-for-google-cloud-genmedia)
- [MCP Toolbox for Databases Documentation](https://google.github.io/adk-docs/mcp/databases/)
